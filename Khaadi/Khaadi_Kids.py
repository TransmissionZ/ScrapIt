...
length=requests.get("https://www.khaadi.com/pk/kids.html?p=2&product_list_limit=48").text
...
#csv file creation
if(os.path.exists("Khaadi_Kids.csv")):
       os.remove("Khaadi_Kids.csv")
ptr=open("Khaadi_Kids.csv","a+")
...
while(loop_index <= int(lens.text)):
    ...
    #html=page.read()
    html=requests.get("https://www.khaadi.com/pk/kids.html?p="+str(loop_index)+"&product_list_limit=48").text
    ...
    for item in pages:
        ...
        if(temp is not None):
            ...
            for data in temp:
                 ...
                 urllib.request.urlretrieve(images,"Images_Kids/"+str(index)+"-"+str(inner_index)+".jpg")
                 inner_index+=1
        else:
            temp=parse.find("img",itemprop="image")
            urllib.request.urlretrieve(temp["src"],"Images_Kids/"+str(index)+"-"+"0.jpg")
        '''
        #get sizes
        '''
        size_str=""
